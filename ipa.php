...
function print_chart($reference) {
	...
	for ($i = 0; $i < ($x / 2); $i++) {
		...
		print('<th colspan="2" class="rotate" id="'.$lookup[$k][1].'"><input type="checkbox" checked="checked" style="display: none;"><div><a target="_blank" href="'.$lookup[$k][2].'" title="'.$caption.'">'.$lookup[$k][1].'</a></div></input></th>');
		$k++;
	}
	print('</tr>');
	for ($j = 0; $j < $y; $j++) {
		...
		print("<th class='side' id='".$lookup[$k][1]."'><input type='checkbox' checked='checked' style='display: none;'><div><a target='_blank' href='".$lookup[$k][2]."' title='".$caption."'>".$lookup[$k][1]."</a></div></input></th>");
		$k++;
		for ($i = 1; $i < $x + 1; $i++) {
...
function other_tables($name) {
	...
	foreach ($file as $line) {
		...
		foreach ($line as $key => $part) {
			// if ($key == 0) {
			// 	$result .= "<th class=\"clickah\">".$part."</th>";
			// }
			// else
			if ($key % 2 == 0) {
