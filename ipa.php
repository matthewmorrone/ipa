...
function css_standard()
{
    ...
    echo(".play 			{display: none;}");
    echo("table, th, td		{border: 1px solid black; border-collapse: collapse;}");
    //echo(".rotated			{-webkit-transform: rotate(-90deg); -moz-transform: rotate(-90deg);}");
}
function echoecho()
{	
...
function print_chart($reference)
{
	...
	for ($j = 0; $j < $y; $j++)
	{
		...
		print("<th class='side' id='".$lookup[$k][1]."'><input type='checkbox' checked='checked' style='display: none;'><div><a href='".$lookup[$k][2]."' title='".$caption."'>".$lookup[$k][1]."</a></div></input></th>"); $k++;
		for ($i = 1; $i < $x + 1; $i++)
		{
